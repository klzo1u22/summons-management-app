...
// Schema with STRICT progressive validation
const strictSummonSchema = z.object({
    ...
    // 4. RESCHEDULE REQUESTED requires IS SERVED
...
export type StrictSummonFormData = z.infer<typeof strictSummonSchema>;

interface StrictSummonFormProps {
    onSubmit?: (data: StrictSummonFormData) => void;
    onCancel?: () => void;
    initialData?: Summons;
    cases?: Case[];
}

export function StrictSummonForm({ onSubmit: propsOnSubmit, onCancel, initialData, cases = [] }: StrictSummonFormProps) {
    const router = useRouter();
    const [step, setStep] = useState(1);
    const prevStep = () => setStep(s => Math.max(1, s - 1));

    const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<StrictSummonFormData>({
        resolver: zodResolver(strictSummonSchema),
        defaultValues: (initialData ? {
            ...
            mode_of_service: [],
            purpose: [],
        }) as StrictSummonFormData
    });

    const isIssued = watch('is_issued');
    const isServed = watch('is_served');
    const requestsReschedule = watch('requests_reschedule');

    // AUTO-CLEAR LOGIC & PROGRESSIVE SYNC
    useEffect(() => {
        if (!isIssued) {
            setValue('is_served', false);
            setValue('served_date', '');
            setValue('mode_of_service', []);
        }
    }, [isIssued, setValue]);

    useEffect(() => {
        if (!isServed) {
            setValue('requests_reschedule', false);
            setValue('rescheduled_date', '');
        }
    }, [isServed, setValue]);

    useEffect(() => {
        if (!requestsReschedule) {
            setValue('rescheduled_date_communicated', false);
        }
    }, [requestsReschedule, setValue]);

    const onSubmit = async (data: StrictSummonFormData): Promise<void> => {
        try {
            if (propsOnSubmit) {
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="rounded-xl border border-border bg-card shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className={cn("transition-opacity", !isIssued && "opacity-50 grayscale pointer-events-none")}>
                                ...
                                {errors.issue_date && <p className="text-destructive text-xs mt-1">{errors.issue_date.message}</p>}
                            </div>
                            ...
                            <div className={cn("transition-opacity", !requestsReschedule && "opacity-50 grayscale pointer-events-none")}>
                    ...
                    <div className="rounded-xl border border-border bg-card shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className={cn("transition-opacity", !isServed && "opacity-50 grayscale pointer-events-none")}>
                                <Label htmlFor="served_date" className="text-muted-foreground">Served Date</Label>
                                <Input type="date" id="served_date" {...register('served_date')} className="bg-background" />
                                {errors.served_date && <p className="text-destructive text-xs mt-1">{errors.served_date.message}</p>}
                            </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div className="rounded-xl border border-border bg-card shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-6">
                            {[
                                { id: 'is_issued', label: 'Summon Issued', parent: null },
                                { id: 'is_served', label: 'Summon Served', parent: 'is_issued' },
                                { id: 'requests_reschedule', label: 'Reschedule Requested', parent: 'is_served' },
                                { id: 'rescheduled_date_communicated', label: 'Date Communicated', parent: 'requests_reschedule' },
                                ...
                                const parentValue = flag.parent ? watch(flag.parent as any) : true;
                                ...
                                return (
                                    <div
                                        key={flag.id}
                                        className={cn(
                                            "flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/20 transition-all group",
                                            isEnabled ? "hover:border-primary/50 hover:bg-muted/40 cursor-pointer" : "opacity-40 grayscale pointer-events-none"
                                        )}
                                        onClick={() => isEnabled && setValue(flag.id as any, !watch(flag.id as any))}
                                    >
                                        <input
                                            type="checkbox"
                                            id={flag.id}
            ...
            <div className="fixed bottom-0 left-[var(--sidebar-width)] right-0 bg-background/95 backdrop-blur-md px-10 py-5 border-t border-border z-50 flex items-center justify-between gap-4 transition-all shadow-lg">
                <div className="flex flex-col gap-1">
                    <p className="text-xs text-muted-foreground italic">
                        * Strict validation mode active. Fields enable progressively based on status.
                    </p>
                ...
                <div className="flex gap-4">
                    ...
                    {step < 3 ? (
                        <Button
                            type="button"
                            onClick={async () => {
                                // Validate current step fields? Zod superRefine handles cross-field logic but for single step it's complex.
                                // We'll just continue as per original design but with visual disabling.
